...
final routeAlertsProvider = FutureProvider.autoDispose
    .family<List<RouteAlert>, WeatherTimelineEtaRequest>((ref, req) async {
      final profile = ref.watch(profileProvider);
      final Map<String, double> thresholds = Map<String, double>.from(
        ref.watch(alertThresholdsProvider).values,
      );
      final timeline = await ref.watch(weatherTimelineEtaProvider(req).future);
      return const EvaluateRouteAlerts()(
        profile: profile.copyWith(alertThresholds: thresholds),
        timeline: timeline,
      );
    });
