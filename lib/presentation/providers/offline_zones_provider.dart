...
part 'offline_zones_state.freezed.dart';
...
class OfflineZonesNotifier extends AsyncNotifier<OfflineZonesState> {
  ...
  Future<List<OfflineZone>> _loadInitialZones() async {
    try {
      ...
      AppLogger.error('Failed to load initial zones', name: 'OfflineZonesNotifier', error: e, stackTrace: st);
      rethrow;
    }
  }
  ...
  Future<void> _persist(List<OfflineZone> zones) async {
    try {
      ...
      AppLogger.info('Successfully persisted ${zones.length} offline zones', name: 'OfflineZonesNotifier');
    } catch (e, st) {
      AppLogger.error('Failed to persist offline zones', name: 'OfflineZonesNotifier', error: e, stackTrace: st);
  ...
  Future<bool> add({
    ...
    if (state.value == null) {
      AppLogger.warning('Attempted to add zone while not initialized', name: 'OfflineZonesNotifier');
      return false;
    }
    
    final validationError = _validateZoneData(name, lat, lng, radiusKm);
    if (validationError != null) {
      AppLogger.warning('Invalid zone data: $validationError', name: 'OfflineZonesNotifier');
      return false;
    }
    ...
    try {
      ...
      AppLogger.info('Successfully added offline zone: ${newZone.name}', name: 'OfflineZonesNotifier');
      return true;
    } catch (e, st) {
      AppLogger.error('Failed to add offline zone', name: 'OfflineZonesNotifier', error: e, stackTrace: st);
      state = AsyncValue.data(state.value!.copyWith(isLoading: false, error: 'Failed to add zone'));
      return false;
    }
  }
  
  String? _validateZoneData(String name, double lat, double lng, double radiusKm) {
    final trimmedName = name.trim();
    if (trimmedName.isEmpty) return 'Name cannot be empty';
    if (trimmedName.length > 100) return 'Name too long (max 100 characters)';
    
    if (lat.isNaN || lng.isNaN || radiusKm.isNaN) return 'Invalid coordinates or radius';
    if (lat < -90 || lat > 90) return 'Latitude must be between -90 and 90';
    if (lng < -180 || lng > 180) return 'Longitude must be between -180 and 180';
    if (radiusKm <= 0 || radiusKm > 1000) return 'Radius must be between 0 and 1000 km';
    
    return null;
  }
  
  OfflineZone _createZone(String name, double lat, double lng, double radiusKm) {
  ...
  Future<bool> remove(String id) async {
    if (state.value == null) {
      AppLogger.warning('Attempted to remove zone while not initialized', name: 'OfflineZonesNotifier');
      return false;
    }
    
    final zoneToRemove = state.value!.zones.where((z) => z.id == id).firstOrNull;
    if (zoneToRemove == null) {
      AppLogger.warning('Zone not found for removal: $id', name: 'OfflineZonesNotifier');
      return false;
    }
    ...
    try {
      ...
      AppLogger.info('Successfully removed offline zone: ${zoneToRemove.name}', name: 'OfflineZonesNotifier');
      return true;
    } catch (e, st) {
      AppLogger.error('Failed to remove offline zone', name: 'OfflineZonesNotifier', error: e, stackTrace: st);
      state = AsyncValue.data(state.value!.copyWith(isLoading: false, error: 'Failed to remove zone'));
      return false;
    }
  }
  ...
  Future<bool> update(String id, {
    ...
    if (state.value == null) {
      AppLogger.warning('Attempted to update zone while not initialized', name: 'OfflineZonesNotifier');
      return false;
    }
    
    final existingZone = state.value!.zones.where((z) => z.id == id).firstOrNull;
    if (existingZone == null) {
      AppLogger.warning('Zone not found for update: $id', name: 'OfflineZonesNotifier');
      return false;
    }
    ...
    if (validationError != null) {
      AppLogger.warning('Invalid updated zone data: $validationError', name: 'OfflineZonesNotifier');
      return false;
    }
    ...
    try {
      ...
      AppLogger.info('Successfully updated offline zone: ${updatedZone.name}', name: 'OfflineZonesNotifier');
      return true;
    } catch (e, st) {
      AppLogger.error('Failed to update offline zone', name: 'OfflineZonesNotifier', error: e, stackTrace: st);
      state = AsyncValue.data(state.value!.copyWith(isLoading: false, error: 'Failed to update zone'));
      return false;
    }
  }
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      AppLogger.error('Failed to refresh offline zones', name: 'OfflineZonesNotifier', error: e, stackTrace: st);
      state = AsyncValue.data(state.value!.copyWith(
        isLoading: false,
...
final offlineZonesProvider = AsyncNotifierProvider<OfflineZonesNotifier, OfflineZonesState>(
