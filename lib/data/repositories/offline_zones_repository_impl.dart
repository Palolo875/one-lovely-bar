...
class OfflineZonesRepositoryImpl implements OfflineZonesRepository {
  ...
  List<OfflineZone> read() {
    try {
      ...
      AppLogger.error(
        'Failed to read offline zones from storage',
    ...
    try {
      return _settings
          ...
          .handleError((e, st) {
            ...
            AppLogger.error(
              'Error in offline zones watch stream',
              name: 'OfflineZonesRepositoryImpl',
              error: e,
      ...
      AppLogger.error(
        'Failed to setup offline zones watch stream',
        name: 'OfflineZonesRepositoryImpl',
        error: e,
        stackTrace: st,
      );
  ...
  Future<void> save(List<OfflineZone> zones) async {
    try {
      if (zones.length > 1000) {
        throw ArgumentError(
          'Too many zones (${zones.length}). Maximum allowed is 1000.',
        );
      }
      ...
      AppLogger.info(
        'Successfully saved ${zones.length} offline zones',
        name: 'OfflineZonesRepositoryImpl',
      );
    } catch (e, st) {
      AppLogger.error(
        'Failed to save offline zones to storage',
        name: 'OfflineZonesRepositoryImpl',
        error: e,
        stackTrace: st,
      );
...
Map<String, Object?> _encode(OfflineZone zone) {
  try {
    ...
    AppLogger.error(
      'Failed to encode offline zone: ${zone.id}',
      name: '_encode',
      error: e,
    );
    rethrow;
...
List<OfflineZone> _decode(Object? raw) {
  ...
  if (raw is! List) {
    AppLogger.warning(
      'Invalid offline zones data format: expected List, got ${raw.runtimeType}',
      name: '_decode',
    );
    return const <OfflineZone>[];
  }
  ...
  for (var i = 0; i < raw.length; i++) {
    try {
      ...
      if (zone != null) {
        ...
        invalidCount++;
        AppLogger.warning(
          'Invalid offline zone data at index $i',
          name: '_decode',
        );
      }
    } catch (e, st) {
      invalidCount++;
      AppLogger.error(
        'Error decoding offline zone at index $i',
        name: '_decode',
        error: e,
        stackTrace: st,
      );
  ...
  if (invalidCount > 0) {
    AppLogger.warning(
      'Skipped $invalidCount invalid offline zone entries during decoding',
      name: '_decode',
    );
  }

  AppLogger.debug(
    'Successfully decoded ${zones.length} offline zones',
    name: '_decode',
  );
  return zones;
}
...
OfflineZone? _decodeOne(Object? raw) {
  ...
  try {
    ...
    AppLogger.error(
      'Unexpected error decoding offline zone',
      name: '_decodeOne',
      error: e,
      stackTrace: st,
    );
